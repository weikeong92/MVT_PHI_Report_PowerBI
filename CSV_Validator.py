        ...
        with open(csv_file, 'r', newline='') as file:
            ...
            for row in rows[1:]:
                ...
                if row[qty_idx].strip() == '-' or row[qty_idx].strip() == "#VALUE!":
                    row[qty_idx] = '0'
                elif row[qty_idx].startswith("-") and row[qty_idx][1:].isdigit():
                    row[qty_idx] = '0'

                yield_idx = header.index('Yield')
                if row[yield_idx].strip() == "#VALUE!" or row[yield_idx].strip() == "#DIV/0!":
                    row[yield_idx] = '0%'
                elif row[yield_idx].startswith("-") and row[yield_idx][1:].isdigit():
                    row[yield_idx] = '0'
...
def main():
    ...
    folder_path = "O:\\yield_report\\LatestData\\LatestData\\2024\\07\\data"
